...
final class CameraService: NSObject, ObservableObject {
    ...
    private func configureSession() {
        sessionQueue.async {
            ...
            // Use a steady 1280×720 VIDEO-analysis preset instead of `.photo`.
            // `.photo` runs the continuous still-capture pipeline whose AE/AF/AWB
            // hunt every frame — frame-to-frame brightness/sharpness drift pushes
            // borderline pills across the confidence threshold (count 29↔30) and
            // breathes the tray/chute segmentation edges. A fixed video stream is
            // what Android uses (targetResolution 1280×720) and is the input
            // stability the detection logic was tuned against. Guarded by
            // canSetSessionPreset so it falls back to `.photo` on the rare device
