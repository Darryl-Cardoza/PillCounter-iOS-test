...
struct FaceRecognitionRepositoryTests {
    ...
    /// `registerUser` takes first/last name separately and stores the
    /// combination as `name` ("\(firstName) \(lastName)"). These tests assert
    /// against the combined value, so split on the first space to reconstruct
    /// exactly the string the store will build.
    private static func register(
        _ repo: FaceRecognitionRepository, name: String
    ) -> FaceUserEntity {
        let parts = name.split(separator: " ", maxSplits: 1).map(String.init)
        return repo.registerUser(firstName: parts[0], lastName: parts.count > 1 ? parts[1] : "")
    }
    ...
    @Test func registerUserCreatesActiveUser() {
        ...
        let user = Self.register(repo, name: name)
    ...
    @Test func saveEnrollmentEmbeddingsPersistsAllSamples() {
        let repo = FaceRecognitionRepository.shared
        let user = Self.register(repo, name: "Repo Save \(UUID().uuidString.prefix(8))")
    ...
    @Test func saveEnrollmentEmbeddingsFailsForEmptyList() {
        let repo = FaceRecognitionRepository.shared
        let user = Self.register(repo, name: "Repo Empty \(UUID().uuidString.prefix(8))")
        guard let userId = user.id else {
            Issue.record("expected user id")
            return
        }
